...
##### Page Table
- OS가 관리
- 한 프로세스 당 하나의 Page Table
##### 문제점
- External fragmentation은 없지만 Internal Fragmentation은 발생 가능
#### Paging 예제
...
- 이 경우, 주소 연산 때문에 2번의 메모리 접근이 일어나고 이는 오버헤드로 이어짐
#### Associative Memory
- 기존 메모리는 여러 데이터를 한 번에 찾는 것이 불가능
- 하나하나 접근했어야 하나, AM은 병렬적으로 찾는 것이 가능
#### TLB
